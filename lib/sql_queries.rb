...
"select projects.title, sum(pledges.amount) from projects inner join pledges on pledges.project_id = projects.id group by title"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"select name,age,sum(pledges.amount) from users inner join pledges on pledges.user_id = users.id group by name"
end
